...
.then(p => message.channel.send(`<a:emoji18:808469005249151037> ${message.author}, Vous avez définis le statut de votre bot en \`${str_content}\`.`))
.catch(e => { message.channel.send(`<a:emoji4:806652395852791858> ${message.author}, Une erreur a été rencontré. \n **Plus d'informations:** \`🔻\` \`\`\`${e}\`\`\``); });

} else {
        message.channel.send(`<a:emoji4:806652395852791858> ${message.author}, Vous avez fournie aucune valeur, veuillez recommencer cette commande en ajoutant ce que vous souhaitez que votre bot ais comme statut`);
    }
};
